...
export default function MessagesPage() {
  ...
  }, [searchParams, useSwaggerAPI, refetchConversations]);

  useEffect(() => {
    if (useSwaggerAPI) {
      // Handle Swagger conversation selection
  ...
  }, [propertyId, receiverId, property, conversations, swaggerConversations, propertyLoading, useSwaggerAPI, user?.id]);
  ...
  const handleSendMessage = async () => {
    ...
    if (useSwaggerAPI && selectedConversation) {
      ...
      // Legacy API handling (when not using Swagger API or no conversation selected)
      const tempMessage: ExtendedSwaggerMessage = {
        id: `temp-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
        conversation_id: selectedConversation || "",
        content: messageContent,
  ...
  const handleRetryMessage = async (message: ExtendedSwaggerMessage) => {
    ...
    //     }
    //   }
    // }, [currentConversations, conversationMessages]);
                    ...
                    {(() => {
                      if (useSwaggerAPI) {
                        ...
                        const otherUser = conversations.find(
                          (c) => c.property.id === selectedConversation
                        )?.other_user;
                        return otherUser?.avatar ? (
                          <Image
                            src={otherUser?.avatar || ""}
            ...
            <div className="flex-1 flex flex-col items-center justify-center bg-gray-50 p-8">
              <div className="text-center">
                ...
                <p className="text-gray-500 max-w-sm">
                  Choose a conversation from the sidebar to start messaging with guests.
                </p>
              </div>
            </div>
          )}
